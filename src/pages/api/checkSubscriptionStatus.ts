...
export default async function handler(req: { method: string; body: { paymentId: any; }; }, res: { status: (arg0: number) => { (): any; new(): any; json: { (arg0: { message: string; }): void; new(): any; }; end: { (arg0: string): void; new(): any; }; }; setHeader: (arg0: string, arg1: string[]) => void; }) {
   if (req.method === 'POST') {
      const { paymentId } = req.body;
      console.log("Recebido paymentId para verificação:", paymentId);

      const apiUrl = `https://asaas.com/api/v3/subscriptions/${paymentId}`;
